...
function ptLoadMemberships() {
    ...
    fetch('/api/pt/memberships')
        ...
        .then(function (list) {
            tbody.innerHTML = '';
            if (!list.length) {
                tbody.innerHTML = '<tr><td colspan="9" class="pt-empty"><i class="fas fa-clipboard-list"></i>No memberships yet. Use the form above to create one.</td></tr>';
                return;
            }
            list.forEach(function (m) {
                var payClass = m.paymentStatus === 'active' ? 'pt-badge-paid' : 'pt-badge-pending';
                var activeClass = m.paymentStatus === 'active' ? 'pt-badge-active' : 'pt-badge-inactive';
                var activeLabel = m.paymentStatus === 'active' ? 'Active' : 'Suspended';
                var payLabel = m.paymentStatus === 'active' ? 'Active' : (m.paymentStatus || 'pending');
                var payLabelClass = m.paymentStatus === 'active' ? 'pt-badge-paid' : 'pt-badge-pending';
                var tr = document.createElement('tr');
                tr.innerHTML =
                    ...
                    '<td><span class="pt-badge ' + payLabelClass + '">' + payLabel + '</span></td>' +
                    '<td><span class="pt-badge ' + activeClass + '">' + activeLabel + '</span></td>' +
                    '<td>' +
        ...
        .catch(function () {
            tbody.innerHTML = '<tr><td colspan="9" class="pt-empty" style="color:#f87171;"><i class="fas fa-exclamation-circle"></i> Failed to load memberships.</td></tr>';
        });
}
